...
public class SportsmenViewModel : BaseUserControlViewModel
{
    ...
    {
        _repository.CreateSportsman(FullName, Age, PhoneNumber, Specialization.Id, Coach.SportClubId, Coach.Id);
    ...
    private void OnSaveRecordCommandExecute(object p)
    {
        ...
        Sportsman.UpdateInfo(FullName, Age, PhoneNumber, Specialization.Id, Coach.SportClubId, Coach.Id);
        UpdateData();
    }
    
    public ICommand SelectedSpecializationCommand { get; }
        
    private bool CanSelectedSpecializationCommandExecute(object p) => true;

    private void OnSelectedSpecializationCommandExecute(object p)
    {
        if(Specialization != null)
            Coaches = _repository.GetCoachesBySpecializationId(Specialization.Id);
    }
    ...
    public SportsmenViewModel() : base()
    {
        ...
        SelectedSpecializationCommand = new LambdaCommand(OnSelectedSpecializationCommandExecute,
            CanSelectedSpecializationCommandExecute);
